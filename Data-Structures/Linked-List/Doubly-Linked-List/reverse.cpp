...
void reverseIterative(Node *&head) {
  ...
  Node *temp = NULL; // Create a temporary node pointer to store the previous
                     // node during reversal
  while (curr != NULL) { // Loop through the list until the end is reached
    ...
    curr->prev = curr->next; // Swap the previous and next node pointers of the
                             // current node
    curr->next = temp; // Set the next node pointer of the current node to the
                       // temporary node pointer
    curr = curr->prev; // Move to the next node in the list
  }

  // Set the head pointer to the new head of the reversed list
  head = temp->prev; // The previous node of the last node processed will be the
...
void reverseUsingStack(Node *&head) {
  ...
  while (!nodes.empty()) {
    ...
    prev = current; // Set the current node as the previous node for the next
                    // iteration
  }

  // Set the tail of the DLL to nullptr
  prev->next =
      NULL; // The last node processed in the loop is the new tail of the DLL
}
