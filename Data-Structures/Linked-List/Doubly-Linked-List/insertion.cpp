...
void push(Node *&head, int x) {
  ...
  n->next = head;
  // If the head is not NULL, set the previous pointer of the current head to
  // the new Node
  if (head != NULL) {
    head->prev = n;
  }
...
// This function inserts a new Node with the given value after a given Node in a
// doubly linked list
void insertAfter(Node *prevNode, int x) {
  ...
  new_node->next = prevNode->next; // Set the next pointer of the new Node to
                                   // the next pointer of the previous Node
  prevNode->next =
      new_node; // Set the next pointer of the previous Node to the new Node
  new_node->prev =
      prevNode; // Set the prev pointer of the new Node to the previous Node
  // Update the prev pointer of the next Node, if it exists
  if (new_node->next != NULL) {
    new_node->next->prev =
        new_node; // Set the prev pointer of the next Node to the new Node
  }
}

// This function appends a new Node with the given value to the end of a doubly
// linked list
void append(Node *&head, int x) {
  // Allocate memory for the new node and set its value to 'x'
  Node *n = new Node(x);
  // Create a temporary node and initialize it to the first node of the linked
  // list
  Node *temp = head;
  // If the linked list is empty, set the first node to be the new node and
  // return
  if (head == NULL) {
    head = n;
    return;
...
int main() {
  ...
  insertAfter(head->next,
              5); // insert new node after the next to next node of the head
  insertAfter(head, 1);
