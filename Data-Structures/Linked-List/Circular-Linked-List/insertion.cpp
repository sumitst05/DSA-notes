...
void insertAfter(Node* head, int pos, int x) {
    ...
    // Traverse the list to find the node before the position where the new node is to be inserted
    Node *temp = head;
    int count = 1;
    ...
    // If the position is greater than the number of nodes in the list, insert the new node at the end of the list
    if (count < pos - 1) {
        temp->next = n;
        n->next = head;
    }
