...
void swap(Node* &head, int x, int y) {
    ...
    while (currX->data != x && currX->next != head) { // while loop to find node with data x
        prevX = currX; // set previous node to current node
        currX = currX->next; // move to the next node
    }

    Node *prevY = NULL, *currY = head;
    while (currY->data != y && currY->next != head) { // while loop to find node with data y
    ...
    if (currX->next == currY) { // if the nodes are adjacent to each other
        ...
        currY->next = currX->next; // update next node pointer of y node to the next node of x node
        currX->next = temp; // update next node pointer of x node to the temporary pointer
