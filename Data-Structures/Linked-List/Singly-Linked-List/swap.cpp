...
void swap(Node *&head, int a, int b) {
  if (a == b)
    return; // If a and b are the same, no need to swap
  ...
  if (currA == NULL || currB == NULL)
    return; // If either node is not found, return
  ...
  currB->next = currA->next; // Set the next node of the node containing b to
                             // the next node of the node containing a
  currA->next = temp; // Set the next node of the node containing a to the next
                      // node of the node containing b
}
