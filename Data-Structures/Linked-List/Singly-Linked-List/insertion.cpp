...
void insertAtHead(Node *&head, int x) {
  Node *n =
      new Node(x); // Allocate memory for the new node and set its data field
...
void insertAfter(Node *head, int key, int x) {
  ...
  // If the key does not match the first node, search for it in the rest of the
  // list
  Node *temp = head;
  while (temp->data != key) {
    temp = temp->next;
    // If we reach the end of the list without finding the key, exit the
    // function
    if (temp == NULL) {
      return;
    }
...
void insertAtEnd(Node *head, int x) {
  Node *n = new Node(x);
  // If the list is currently empty, make the new node the head of the list and
  // exit
  if (head == NULL) {
    head = n;
    return;
