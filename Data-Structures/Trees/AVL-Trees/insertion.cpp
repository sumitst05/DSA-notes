...
Node* leftRotate(Node* x) {
    ...
    Node* Tl = y->left;     // Tl is the left child of y (subtree that needs to be moved)
...
Node* rightRotate(Node* y) {
    ...
    Node* Tr = x->right;    // Tr is the right child of x (subtree that needs to be moved)
...
Node* insertNode(Node* node, int value) {
    ...
    // Left Left Case
    // when the tree is already left-heavy and we want to insert in the left left of the parent
    if (balance > 1 && value < node->left->data) {
        return rightRotate(node);
    }

    // Right Right Case
    // when the tree is already right-heavy and we want to insert in the right right of the parent
    if (balance < -1 && value > node->right->data) {
        return leftRotate(node);
    }

    // Left Right Case
    // when the tree is already left-heavy and we want to insert in the left right of the parent
    if (balance > 1 && value > node->left->data) {
    ...
    // Right Left Case
    // when the tree is already right-heavy and we want to insert in the right left of the parent
    if (balance < -1 && value < node->right->data) {
