...
void insertRoot(char key) {
    if(binTree[0] != '\0') cout << "The tree already has a root" << '\n';
    else binTree[0] = key;
}

void insertLeft(char key, int parent) {
    if(binTree[parent] == '\0') {
        cout << "Cannot set child node at " << 2 * parent + 1 << ". No parent found!" << '\n';
    } else {
...
void insertRight(char key, int parent) {
    if(binTree[parent] == '\0') {
        cout << "Cannot set child node at " << 2 * parent + 2 << ". No parent found!" << '\n';
    } else {
