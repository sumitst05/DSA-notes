...
public:
  ...
  bool isEmpty() { // check if the queue is empty
    if (front == -1 &&
        rear == -1) // if front and rear are -1, then the queue is empty
      return true;
    else
      return false;
  }
  ...
  bool isFull() {             // check if the queue is full
    if (rear == MAX_SIZE - 1) // if rear is at the last index of the array, then
                              // the queue is full
      return true;
    else
      return false;
  ...
  void dequeue() {   // remove an element from the front of the queue
    if (isEmpty()) { // if the queue is empty, then return
      ...
      return;
                                ...
                                // then reset front and rear to -1
      front = -1;
      rear = -1;
    }
  ...
  void printQueue() { // print the elements of the queue
    ...
    for (int i = front; i <= rear;
         i++) { // loop through the elements of the queue from front to rear
...
int main() {
  ...
  cout << "Rear element is: " << q.getRear()
       << endl; // get the element at the rear of the queue
