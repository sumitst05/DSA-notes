...
public:
  ...
  bool isEmpty() {
    return (front == NULL); // Return true if the front pointer is NULL
  ...
  void enqueue(int x) {
    ...
    rear->next = newNode; // Set the next pointer of the current rear Node to
                          // the new Node
...
int main() {
  ...
  cout << "Element at front: " << q.peek()
       << endl;   // Get the new element at the front of the Queue
  q.printQueue(); // Print all elements in the queue
